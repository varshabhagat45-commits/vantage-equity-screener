export type LiveQuote = { price: number; chg1d: number; asOf: number };
...
const UA = "Mozilla/5.0 (compatible; VantageScreener/1.0; +https://grok.com) AppleWebKit/537.36";
...
function parseSparkItem(item: {
  symbol?: string;
  response?: Array<{ meta?: { regularMarketPrice?: number; chartPreviousClose?: number; previousClose?: number; regularMarketTime?: number } }>;
  ...
  return {
    ...
    quote: { price: Math.round(price * 100) / 100, chg1d: Math.round(chg1d * 100) / 100, asOf },
  };
}
...
async function fetchSparkChunk(tickers: string[]): Promise<Record<string, LiveQuote>> {
  ...
  const data = (await res.json()) as { spark?: { result?: Parameters<typeof parseSparkItem>[0][] } };
...
export async function fetchNseQuotes(tickers: string[]): Promise<Record<string, LiveQuote>> {
  ...
  for (const group of chunk(unique, 15)) Object.assign(quotes, await fetchSparkChunk(group));
  return quotes;
}
