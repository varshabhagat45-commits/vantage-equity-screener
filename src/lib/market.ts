...
type Cache = { snapshot: MarketSnapshot; storedAt: number };
const TTL_MS = 15 * 60 * 1000;
let cache: Cache | null = null;

function emptySnapshot(): MarketSnapshot {
  return { quotes: {}, asOf: null, fetchedAt: Date.now(), covered: 0, universe: STOCKS.length, source: "snapshot" };
}
...
export const getMarketQuotes = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<MarketSnapshot> => {
    const now = Date.now();
    if (!data.force && cache && now - cache.storedAt < TTL_MS) return cache.snapshot;
    try {
      ...
      const snapshot: MarketSnapshot = {
        quotes: Object.fromEntries(Object.entries(quotes).map(([ticker, q]) => [ticker, { price: q.price, chg1d: q.chg1d }])),
      ...
      return { ...fallback, fetchedAt: now, error: err instanceof Error ? err.message : "Price feed unavailable" };
    }
  });
