      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:gap-6 sm:px-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            disabled={loading}
            className="inline-flex min-h-10 items-center gap-2 rounded-md border border-border bg-surface px-3 text-xs text-muted hover:text-fg disabled:opacity-60"
            title={marketError ?? "Refresh NSE last prices"}
          >
            ...
            <span className="text-left leading-tight">
              ...
              <span className="block tabular-nums">
                {loading
                  ...
                  : marketStatus === "live"
                    ? `${formatIst(marketAsOf)} IST · ${marketCovered}`
                    : "Using last filed snapshot"}
              </span>
            </span>
          </button>

          <Button
            variant={watchOnly ? "secondary" : "outline"}
            size="sm"
            onClick={() => setWatchOnly(!watchOnly)}
            className={cn("shrink-0", watchOnly && "ring-1 ring-accent/20")}
          >
